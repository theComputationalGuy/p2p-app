...
clientIP = '192.168.92.167'
clientPortNumber = 8901
# clientIP = 'localhost'
client_addr = (clientIP, clientPortNumber)

clientSocket.bind((hostIp, hostPortNumber))


window = Tk()
window.title("Connected To: "+ hostIp+ ":"+str(hostPortNumber))
...
def display_image(file_path):
    ...
    my_image.thumbnail((200,200), Image.ANTIALIAS)
    # img.append(ImageTk.PhotoImage(img_resized))
    img.append(ImageTk.PhotoImage(my_image))
    txtMessages.insert(END, "\nPeer : ")
    # txtMessages.insert(END, '\n')
    txtMessages.image_create(END, image = img[n])
    n += 1
